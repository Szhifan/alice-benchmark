from transformers import BertPreTrainedModel, BertModel, AutoTokenizer, AutoConfig, PretrainedConfig
...
for jj in range(1, 4, 1):
    dataset = read_dataset(f"ALICE_train_lc_{jj}.xlsx")  # needs to contain rubrics, question, answer, label
...
#        max_grad_norm=1.0,
    ...
    model = PointerRubricModel.from_pretrained('deepset/gbert-large')
    ...
    model.save_pretrained(f"./grasp-lc-{jj}")
    tokenizer.save_pretrained(f"./grasp-lc-{jj}")
