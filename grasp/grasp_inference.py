...
model = PointerRubricModel.from_pretrained('./grasp-1')
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = model.to(device)
model.eval()

for task in ['UA', 'UQ']:
    dataset = read_dataset(f"alice_data/ALICE_{task}_new.xlsx")  # needs to contain rubrics, question, answer, label
    df = pd.read_excel(f"alice_data/ALICE_{task}_new.xlsx")
    ...
    pd.DataFrame.from_dict(dddf).to_excel(f'{task}_grasp_bert_results.xlsx')
