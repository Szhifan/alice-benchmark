...
class PointerRubricModel(ElectraPreTrainedModel):
...
model = PointerRubricModel.from_pretrained("./grasp-el-3")
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = model.to(device)
model.eval()

for task in ['UA', 'UQ']:
    dataset = read_dataset(f"ALICE_{task}_new.xlsx")  # needs to contain rubrics, question, answer, label
    df = pd.read_excel(f"ALICE_{task}_new.xlsx")

    tokenized_dataset = dataset.map(tokenize_with_spans)
    dataloader = DataLoader(tokenized_dataset, batch_size=4, collate_fn=collator)
    ...
    pd.DataFrame.from_dict(dddf).to_excel(f'{task}_grasp_electra_results.xlsx')
