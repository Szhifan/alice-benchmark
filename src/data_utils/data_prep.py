import torch
from typing import Literal
FIELD_EN2DE = {
    "answer": "Antwort",
    "rubric": "rubric",
    "question": "Frage",
    "sample_solution": "Lösung"
}
## encoding functions 
def basic_encode(example, tokenizer):
    # encode answer only  
    output = tokenizer(example["answer"], max_length=512, truncation=True)
    for field in output:
        example[field] = output[field]
    example["labels"] = int(example["level"])
    return example

def encode_solution_pair(example, tokenizer):
    output = tokenizer(example["sample_solution"], example["answer"], max_length=512, truncation=True) 
    for field in output:
        example[field] = output[field]
    example["labels"] = int(example["level"])
    return example

def encode_sequence_bert(example, tokenizer, fields: list[str] = ["answer","rubric"]): 
    """
    For bert-like models.
    Encode multiple fields with special tokens (SEP).
    """
    def default_encode(example, tokenizer):
        """
        For bert-like models.
        Encode rubric and answer as a sequence pair.
        """
        output = tokenizer(example["answer"], example["rubric"], max_length=512, truncation=True)
        for field in output:
            example[field] = output[field]
        return example
    if set(fields) == {"answer", "rubric"}:
        return default_encode(example, tokenizer)
    text2encode = []
    for field in fields: 
        if field not in example:
            raise ValueError(f"Field '{field}' not found in the example.")
        text2encode.append(example[field])

    text2encode = tokenizer.sep_token.join(text2encode)
    output = tokenizer(text2encode, max_length=512, truncation=True, add_special_tokens=True)
    return output
def encode_sequence_llm(example, tokenizer, fields: list[str] = ["answer","rubric"], add_instruction: bool = False, format: Literal["natural_lang", "structured"] = "natural_lang"):
    """
    For LLM models.
    Encode the fields of the example using the tokenizer with natural language.
    Available fields: answer, question, sample_solution, rubric.
    """
    text2encode = ""
    for field in fields:
        if field not in example:
            raise ValueError(f"Field '{field}' not found in the example.")
        if format == "natural_lang":
            text2encode += f"{FIELD_EN2DE[field]}: {example[field]}\n"
        elif format == "structured":
            text2encode += f"<{FIELD_EN2DE[field]}>{example[field]}</{FIELD_EN2DE[field]}>\n"
    if add_instruction:
        text2encode = "Bestimmen Sie, ob die rubric durch die Antwort erfüllt wird:\n" + text2encode
    output = tokenizer(text2encode, max_length=512, truncation=True)
    for field in output:
        example[field] = output[field]
    return example

def encode_with_spans_bert(example, tokenizer, input_fields: list[str] = ["answer", "rubric"]):
    """
    Encode example with span information
    Tracks spans for rubrics and answer in the tokenized sequence.
    
    Args:
        example: Dict containing the example data
        tokenizer: Tokenizer to use for encoding
        input_fields: List of fields to include in encoding. 
                     Should contain 'answer' and 'rubric' for proper span tracking.
    """
    # Validate required fields for span tracking
    required_fields = {"answer", "rubric"}
    if not required_fields.issubset(set(input_fields)):
        raise ValueError(f"GRASP model requires fields {required_fields}, but got {input_fields}")
    
    # Extract fields based on input_fields parameter
    rubrics = example["rubric"] if isinstance(example["rubric"], list) else [example["rubric"]]
    answer = example["answer"]

    # Step 1: Build the input string while tracking character spans按照input_fields顺序
    input_parts = []
    span_indices_char = []
    answer_start_char = answer_end_char = None

    for field in input_fields:
        if field == "rubric" and field in example:
            # Add rubrics with span tracking
            for rubric in rubrics:
                # Calculate start position in the final joined string
                current_str = tokenizer.sep_token.join(input_parts)
                start_char = len(current_str) + (len(tokenizer.sep_token) if input_parts else 0)
                
                input_parts.append(str(rubric))
                
                # Calculate end position in the final joined string  
                new_str = tokenizer.sep_token.join(input_parts)
                end_char = len(new_str)
                span_indices_char.append((start_char, end_char))

        elif field == "answer" and field in example:
            # Add answer with span tracking
            current_str = tokenizer.sep_token.join(input_parts)
            answer_start_char = len(current_str) + (len(tokenizer.sep_token) if input_parts else 0)
            input_parts.append(str(answer))
            new_str = tokenizer.sep_token.join(input_parts)
            answer_end_char = len(new_str)

        elif field == "question" and field in example:
            # Add optional fields (no span tracking needed)
            input_parts.append(str(example["question"]))
        
        elif field == "sample_solution" and field in example:
            input_parts.append(str(example["sample_solution"]))

    # Join all parts
    input_str = tokenizer.sep_token.join(input_parts)

    # Step 2: Tokenize with offset mapping
    encoding = tokenizer(
        input_str,
        return_offsets_mapping=True,
        truncation=True,
        max_length=512,
    )

    offsets = encoding["offset_mapping"]
    rubric_spans = []

    # Step 3: Map rubric char spans to token spans
    for span_start_char, span_end_char in span_indices_char:
        token_start = token_end = None
        for i, (start, end) in enumerate(offsets):
            if start <= span_start_char < end:
                token_start = i
            if start < span_end_char <= end:
                token_end = i + 1
                break
        if token_start is None:
            token_start = next((i for i, (s, _) in enumerate(offsets) if s >= span_start_char), 0)
        if token_end is None:
            token_end = next((i for i, (_, e) in enumerate(offsets) if e >= span_end_char), len(offsets) - 1) + 1
        rubric_spans.append((token_start, token_end))

    # Step 4: Map answer span to token span (only if answer is in input_fields)
    if "answer" in input_fields and answer_start_char is not None:
        answer_token_start = answer_token_end = None
        for i, (start, end) in enumerate(offsets):
            if start <= answer_start_char < end:
                answer_token_start = i
            if start < answer_end_char <= end:
                answer_token_end = i + 1
                break
        if answer_token_start is None:
            answer_token_start = next((i for i, (s, _) in enumerate(offsets) if s >= answer_start_char), 0)
        if answer_token_end is None:
            answer_token_end = next((i for i, (_, e) in enumerate(offsets) if e >= answer_end_char), len(offsets) - 1) + 1 
        encoding["answer_span"] = [answer_token_start, answer_token_end]
    else:
        # If answer is not included, set dummy span
        encoding["answer_span"] = [0, 0]

    # Step 5: Add to encoding
    encoding["rubric_spans"] = rubric_spans
    encoding["labels"] = int(example["level"])
    
    # Remove offset_mapping as it's not needed for training
    encoding.pop("offset_mapping", None)

    return encoding
def encode_with_spans_llm(example, tokenizer, input_fields: list[str] = ["answer", "rubric"], 
                         add_instruction: bool = False, format: Literal["natural_lang", "structured"] = "natural_lang"):
    """
    Encode example with span information.
    Tracks spans for rubrics and answer in the tokenized sequence.
    
    Args:
        example: Dict containing the example data
        tokenizer: Tokenizer to use for encoding
        input_fields: List of fields to include in encoding. 
                     Should contain 'answer' and 'rubric' for proper span tracking.
        add_instruction: Whether to add instruction text
        format: Format style ("natural_lang" or "structured")
    """
    # Validate required fields for span tracking
    required_fields = {"answer", "rubric"}
    if not required_fields.issubset(set(input_fields)):
        raise ValueError(f"GRASP model requires fields {required_fields}, but got {input_fields}")
    
    # Extract fields based on input_fields parameter
    rubrics = example["rubric"] if isinstance(example["rubric"], list) else [example["rubric"]]
    answer = example["answer"]

    # Step 1: 按input_fields的实际顺序构建文本并追踪span
    text_parts = []
    span_indices_char = []
    answer_start_char = answer_end_char = None
    
    # Add instruction if requested
    if add_instruction:
        instruction = "Bestimmen Sie, ob die rubric durch die Antwort erfüllt wird:\n"
        text_parts.append(instruction)
    for field in input_fields:
        if field == "rubric" and field in example:
            for rubric in rubrics:
                current_text = "".join(text_parts)
                if format == "natural_lang":
                    label_text = f"{FIELD_EN2DE['rubric']}: "
                    start_char = len(current_text) + len(label_text)
                    rubric_text = f"{label_text}{str(rubric)}\n"
                elif format == "structured":
                    label_start = f"<{FIELD_EN2DE['rubric']}>"
                    label_end = f"</{FIELD_EN2DE['rubric']}>"
                    start_char = len(current_text) + len(label_start)
                    rubric_text = f"{label_start}{str(rubric)}{label_end}\n"
                
                text_parts.append(rubric_text)
                
                current_text = "".join(text_parts)
                if format == "natural_lang":
                    end_char = len(current_text) - 1
                else:
                    end_char = len(current_text) - len(f"{label_end}\n")
                
                span_indices_char.append((start_char, end_char))
                
        elif field == "answer" and field in example:
            current_text = "".join(text_parts)
            if format == "natural_lang":
                label_text = f"{FIELD_EN2DE['answer']}: "
                answer_start_char = len(current_text) + len(label_text)
                answer_text = f"{label_text}{str(answer)}\n"
            elif format == "structured":
                label_start = f"<{FIELD_EN2DE['answer']}>"
                label_end = f"</{FIELD_EN2DE['answer']}>"
                answer_start_char = len(current_text) + len(label_start)
                answer_text = f"{label_start}{str(answer)}{label_end}\n"
            
            text_parts.append(answer_text)
            
            current_text = "".join(text_parts)
            if format == "natural_lang":
                answer_end_char = len(current_text) - 1
            else:
                answer_end_char = len(current_text) - len(f"{label_end}\n")
                
        elif field in example:
            if format == "natural_lang":
                field_text = f"{FIELD_EN2DE.get(field, field)}: {example[field]}\n"
            elif format == "structured":
                field_name = FIELD_EN2DE.get(field, field)
                field_text = f"<{field_name}>{example[field]}</{field_name}>\n"
            text_parts.append(field_text)

    # Join all parts
    input_str = "".join(text_parts).rstrip()  # Remove trailing newline

    # Step 2: Tokenize with offset mapping
    encoding = tokenizer(
        input_str,
        return_offsets_mapping=True,
        truncation=True,
        max_length=512,
    )

    offsets = encoding["offset_mapping"]
    rubric_spans = []

    # Step 3: Map rubric char spans to token spans
    for span_start_char, span_end_char in span_indices_char:
        token_start = token_end = None
        for i, (start, end) in enumerate(offsets):
            if start <= span_start_char < end:
                token_start = i
            if start < span_end_char <= end:
                token_end = i + 1
                break
        if token_start is None:
            token_start = next((i for i, (s, _) in enumerate(offsets) if s >= span_start_char), 0)
        if token_end is None:
            token_end = next((i for i, (_, e) in enumerate(offsets) if e >= span_end_char), len(offsets) - 1) + 1
        rubric_spans.append((token_start, token_end))

    # Step 4: Map answer span to token span (only if answer is in input_fields)
    if "answer" in input_fields and answer_start_char is not None:
        answer_token_start = answer_token_end = None
        for i, (start, end) in enumerate(offsets):
            if start <= answer_start_char < end:
                answer_token_start = i
            if start < answer_end_char <= end:
                answer_token_end = i + 1
                break
        if answer_token_start is None:
            answer_token_start = next((i for i, (s, _) in enumerate(offsets) if s >= answer_start_char), 0)
        if answer_token_end is None:
            answer_token_end = next((i for i, (_, e) in enumerate(offsets) if e >= answer_end_char), len(offsets) - 1) + 1
        
        encoding["answer_span"] = [answer_token_start, answer_token_end]
    else:
        # If answer is not included, set dummy span
        encoding["answer_span"] = [0, 0]

    # Step 5: Add to encoding
    encoding["rubric_spans"] = rubric_spans
    encoding["labels"] = int(example["level"])
    
    # Remove offset_mapping as it's not needed for training
    encoding.pop("offset_mapping", None)

    return encoding
# ----------collate functions------------
def base_collate_fn(input_batch, pad_id=0, return_meta=False):
    """
    Basic collate function for batching the input batch.
    Mode: controls whether to return meta information or not.
    """
    input_ids = torch.nn.utils.rnn.pad_sequence([torch.tensor(x["input_ids"]) for x in input_batch], batch_first=True, padding_value=pad_id)
    attention_mask = torch.nn.utils.rnn.pad_sequence([torch.tensor(x["attention_mask"]) for x in input_batch], batch_first=True, padding_value=0)
    labels = torch.tensor([x["labels"] for x in input_batch])
    batch = {
        "input_ids": input_ids,
        "labels": labels,
        "attention_mask": attention_mask,
    } 
    meta = {
        "id": [x["id"] for x in input_batch],
        "level": [x["level"] for x in input_batch],
        "rubric_level": [x.get("rubric_level", None) for x in input_batch],
    }
    if return_meta:
        return batch, meta
    return batch


def xnet_collate_fn(input_batch, pad_id=0, return_meta=False):
    """
    Collate function for grouped dataset where each example contains multiple rubrics
    with input_ids, attention_mask etc. in shape [R, S] format.
    
    Args:
        input_batch: List of examples from grouped dataset
        pad_id: Padding token id
        return_meta: Whether to return metadata
        
    Returns:
        batch: Dict with tensors of shape [B, R, S] where B=batch_size, R=num_rubrics, S=seq_len
        meta: Optional metadata dict
    """
    batch_size = len(input_batch)
    max_rubrics = max([x["num_rubrics"] for x in input_batch])
    # Initialize lists to collect padded sequences
    batch_input_ids = []
    batch_attention_mask = []
    batch_token_type_ids = []
    batch_labels = []
    
    for example in input_batch:
        num_rubrics = example["num_rubrics"]
        # Convert input_ids and attention_mask to tensors
        input_ids_tensors = [torch.tensor(seq) for seq in example["input_ids"]]
        attention_mask_tensors = [torch.tensor(seq) for seq in example["attention_mask"]]
        
        # Pad sequences within this example to same length
        padded_input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids_tensors, batch_first=True, padding_value=pad_id
        )  # [R, S]
        padded_attention_mask = torch.nn.utils.rnn.pad_sequence(
            attention_mask_tensors, batch_first=True, padding_value=0
        )  # [R, S]
        
        # Handle token_type_ids if present
        if "token_type_ids" in example and example["token_type_ids"] is not None:
            token_type_ids_tensors = [torch.tensor(seq) for seq in example["token_type_ids"]]
            padded_token_type_ids = torch.nn.utils.rnn.pad_sequence(
                token_type_ids_tensors, batch_first=True, padding_value=0
            )
        else:
            padded_token_type_ids = None
        
        # Pad rubrics dimension to max_rubrics if needed
        if num_rubrics < max_rubrics:
            seq_len = padded_input_ids.shape[1]
            # Create padding for missing rubrics
            rubric_padding = torch.full((max_rubrics - num_rubrics, seq_len), pad_id, dtype=torch.long)
            padded_input_ids = torch.cat([padded_input_ids, rubric_padding], dim=0)
            
            mask_padding = torch.zeros((max_rubrics - num_rubrics, seq_len), dtype=torch.long)
            padded_attention_mask = torch.cat([padded_attention_mask, mask_padding], dim=0)
            
            if padded_token_type_ids is not None:
                token_type_padding = torch.zeros((max_rubrics - num_rubrics, seq_len), dtype=torch.long)
                padded_token_type_ids = torch.cat([padded_token_type_ids, token_type_padding], dim=0)
        batch_input_ids.append(padded_input_ids)
        batch_attention_mask.append(padded_attention_mask)
        if padded_token_type_ids is not None:
            batch_token_type_ids.append(padded_token_type_ids)
        
        # Use the original labels (not binary)
        batch_labels.append(example["labels"])
    
    # Pad sequence length dimension across batch
    max_seq_len = max(x.shape[1] for x in batch_input_ids)
    
    final_input_ids = []
    final_attention_mask = []
    final_token_type_ids = []
    
    for i in range(batch_size):
        current_seq_len = batch_input_ids[i].shape[1]
        if current_seq_len < max_seq_len:
            # Pad sequence length dimension
            
            seq_padding = torch.full((max_rubrics, max_seq_len - current_seq_len), pad_id, dtype=torch.long)
            padded_input_ids = torch.cat([batch_input_ids[i], seq_padding], dim=1)
            
            mask_seq_padding = torch.zeros((max_rubrics, max_seq_len - current_seq_len), dtype=torch.long)
            padded_attention_mask = torch.cat([batch_attention_mask[i], mask_seq_padding], dim=1)
            
            if batch_token_type_ids:
                token_seq_padding = torch.zeros((max_rubrics, max_seq_len - current_seq_len), dtype=torch.long)
                padded_token_type_ids = torch.cat([batch_token_type_ids[i], token_seq_padding], dim=1)
        else:
            padded_input_ids = batch_input_ids[i]
            padded_attention_mask = batch_attention_mask[i]
            if batch_token_type_ids:
                padded_token_type_ids = batch_token_type_ids[i]
        
        final_input_ids.append(padded_input_ids)
        final_attention_mask.append(padded_attention_mask)
        if batch_token_type_ids:
            final_token_type_ids.append(padded_token_type_ids)
    
    # Stack to create final batch tensors [B, R, S]
    batch = {
        "input_ids": torch.stack(final_input_ids),  # [B, R, S]
        "attention_mask": torch.stack(final_attention_mask),  # [B, R, S]
        "labels": torch.tensor(batch_labels),  # [B] - original labels
        "num_rubrics": torch.tensor([x["num_rubrics"] for x in input_batch]),  # [B] - actual number of rubrics per example,
    }
    
    if final_token_type_ids:
        batch["token_type_ids"] = torch.stack(final_token_type_ids)  # [B, R, S]
    
    meta = {
        "id": [x["id"] for x in input_batch],
        "level": [x["level"] for x in input_batch],
        "question_id": [x["question_id"] for x in input_batch],
        "rubric_level": [x["rubric_level"] for x in input_batch],
    }
    
    if return_meta:
        return batch, meta
    return batch

def grasp_collate_fn(input_batch, pad_id=0, return_meta=False):
    """
    Collate function for GRASP model that handles span information.
    Processes rubric spans, answer spans, and rubric masks.
    
    Args:
        input_batch: List of examples with span information
        pad_id: Padding token id
        return_meta: Whether to return metadata
        
    Returns:
        batch: Dict with input_ids, attention_mask, rubric_spans, answer_span, rubric_mask, labels
        meta: Optional metadata dict
    """
    # Pad input_ids and attention_mask
    input_ids = [torch.tensor(x["input_ids"], dtype=torch.long) for x in input_batch]
    attention_masks = [torch.tensor(x["attention_mask"], dtype=torch.long) for x in input_batch]

    batch_input_ids = torch.nn.utils.rnn.pad_sequence(
        input_ids, batch_first=True, padding_value=pad_id
    )
    batch_attention_mask = torch.nn.utils.rnn.pad_sequence(
        attention_masks, batch_first=True, padding_value=0
    )

    # Handle rubric spans - pad to max number of rubrics
    max_rubrics = max(len(x["rubric_spans"]) for x in input_batch)
    batch_size = len(input_batch)
    
    # Initialize tensors for spans and masks
    rubric_spans_tensor = torch.zeros(batch_size, max_rubrics, 2, dtype=torch.long)
    rubric_mask_tensor = torch.zeros(batch_size, max_rubrics, dtype=torch.bool)
    
    for i, example in enumerate(input_batch):
        spans = torch.tensor(example["rubric_spans"], dtype=torch.long)
        num_spans = spans.shape[0]
        rubric_spans_tensor[i, :num_spans, :] = spans
        rubric_mask_tensor[i, :num_spans] = True

    # Handle answer spans
    answer_spans = torch.tensor([x["answer_span"] for x in input_batch], dtype=torch.long)

    # Create batch dictionary
    batch = {
        "input_ids": batch_input_ids,
        "attention_mask": batch_attention_mask,
        "rubric_spans": rubric_spans_tensor,
        "answer_span": answer_spans,
        "rubric_mask": rubric_mask_tensor,
        "labels": torch.tensor([x["labels"] for x in input_batch], dtype=torch.long),
    }
    

    # Prepare metadata
    meta = {
        "id": [x.get("id", i) for i, x in enumerate(input_batch)],
        "level": [x.get("level", x["labels"]) for x in input_batch],
        "rubric_level": [x.get("rubric_level", None) for x in input_batch],
    }
    
    if return_meta:
        return batch, meta
    return batch