...
def add_training_args(parser):
    ...
    parser.add_argument('--share-encoders', action='store_true', help='share encoders between two tasks')
...
def train_epoch(
    ...
    for epoch in train_iterator:
        train_dataloader = DataLoader(train_dataset, num_workers=0, pin_memory=True, batch_size=args.batch_size, collate_fn=TASK_DATASET.collate_rubric_separate, shuffle=True) 
...
def evaluate(model, dataset, batch_size, is_test=False): 
    dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=TASK_DATASET.collate_rubric_separate, shuffle=False)
...
def main(args):
    ...
    # Load the dataset
    ds = TASK_DATASET(enc_fn=encode_rubric_span_separate)
